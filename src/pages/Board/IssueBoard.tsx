...
export default function IssueBoard() {
    const backlogIssues = useSelector((state: RootState) => state.issues?.backlog);
    const todoIssues = useSelector((state: RootState) => state.issues?.todo);
    const inProgressIssues = useSelector((state: RootState) => state.issues?.inProgress);
    const doneIssues = useSelector((state: RootState) => state.issues?.done);
    const canceledIssues = useSelector((state: RootState) => state.issues?.canceled);
    ...
    return (
        ...
        >
            <div className='flex flex-1 pt-6 pl-8 overflow-scroll bg-gray-100'>
                <IssueCol title={'Backlog'} status={Status.BACKLOG} issues={backlogIssues} />
                <IssueCol title={'Todo'} status={Status.TODO} issues={todoIssues} />
                <IssueCol title={'In Progress'} status={Status.IN_PROGRESS} issues={inProgressIssues} />
                <IssueCol title={'Done'} status={Status.DONE} issues={doneIssues} />
                <IssueCol title={'Canceled'} status={Status.CANCELED} issues={canceledIssues} />
            </div>
        </DragDropContext>
    )
}
