...
function randomIssue(status?: string): Issue {
    var keyPriority = Object.values(Priority)[Math.floor(Math.random() * Object.keys(Priority).length)];
    let keyStatus = status || Object.values(Status)[Math.floor(Math.random() * Object.keys(Status).length)];
    return {
        id: 'GIT-' + generateId(),
...
function updateIssueStatusAndPos(issueId: string, srcStatus: string, destStatus: string, srcPos: number, destPos: number) {
